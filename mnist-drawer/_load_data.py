...
def dataset_mnist_pytorch(normalize=True):
    if normalize:
        trans = transforms.Compose([transforms.ToTensor(), transforms.Normalize((.5,), (1.,))])
    else:
...
def loader_mnist_pytorch(batch_size):

    mnist_train, mnist_test = dataset_mnist_pytorch()
